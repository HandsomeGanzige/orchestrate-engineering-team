...
const REQUIRED = Object.freeze(["SKILL.md", "agents/openai.yaml", "assets/main-work-brief.md", "assets/role-task-packet.md", "references/review-agent-retrievability/SKILL.md", "references/role-contracts.yaml", "references/config.schema.json"]);
const MAIN_RESOURCE_COUNT = 5;
...
export async function checkAgentProfiles(root) {
  ...
  if (diagnostics.length) return { ok: false, diagnostics, summary: { skills: 1, roles: 4, mainResources: MAIN_RESOURCE_COUNT } };
  ...
  for (const [pattern, code] of [[/only public Skill/i,"SINGLE_MAIN"],[/host's native subagent|native subagent/i,"NATIVE_DISPATCH"],[/prompt fallback/i,"FALLBACK"],[/Required capability missing/i,"REQUIRED_CAPABILITY"],[/Never use `.agent-work` as a lock, queue, database, lifecycle, state machine, or quality gate/i,"AGENT_WORK_NOT_STATE"],[/main-work-brief\.md/i,"MAIN_BRIEF"],[/not a particular tool/i,"TOOL_NEUTRAL"],[/not mandatory phases/i,"OPTIONAL_ROLES"],[/configure the team/i,"CONFIGURE"],[/references\/review-agent-retrievability\/SKILL\.md/i,"RETRIEVABILITY_POINTER"],[/immediately before dispatch/i,"RETRIEVABILITY_SELECTIVE_DISCLOSURE"],[/audit remains read-only; Main routes accepted repairs/i,"RETRIEVABILITY_ROUTE"]]) if (!pattern.test(skill)) diagnostics.push(diag(`${MAIN}/SKILL.md`, code, `missing required Main behavior: ${code}`));
  const taskPacket = await readFile(path.join(root, MAIN, "assets/role-task-packet.md"), "utf8");
  if (!/entry points are not the presumed complete scope/i.test(taskPacket) || !/No particular search tool or index implementation is required/i.test(taskPacket)) diagnostics.push(diag(`${MAIN}/assets/role-task-packet.md`, "SELF_DISCOVERY", "role task packet must require tool-neutral repository discovery beyond supplied entry points"));
  const retrievabilitySkillFile = `${MAIN}/references/review-agent-retrievability/SKILL.md`;
  const retrievabilitySkill = await readFile(path.join(root, retrievabilitySkillFile), "utf8");
  for (const [pattern, code] of [[/name: review-agent-retrievability/, "RETRIEVABILITY_NAME"], [/disable-model-invocation: true/, "RETRIEVABILITY_INVOCATION_INTENT"], [/fresh-context perspective/i, "RETRIEVABILITY_FRESH_CONTEXT"], [/deletion test/i, "RETRIEVABILITY_DELETION_TEST"], [/Do not reward document, comment, tag, or anchor count/i, "RETRIEVABILITY_ANTI_NOISE"], [/Main decides whether to accept and route a finding/i, "RETRIEVABILITY_READ_ONLY_ROUTE"]]) if (!pattern.test(retrievabilitySkill)) diagnostics.push(diag(retrievabilitySkillFile, code, `missing retrievability review behavior: ${code}`));
  const brief = await readFile(path.join(root, MAIN, "assets/main-work-brief.md"), "utf8");
  for (const heading of ["Current outcome and acceptance", "User decisions and constraints", "Current progress", "Repository facts and evidence", "Open work and risks", "Next action"]) if (!brief.includes(`## ${heading}`)) diagnostics.push(diag(`${MAIN}/assets/main-work-brief.md`, "BRIEF_SECTION", `missing brief section: ${heading}`));
  ...
  try {
    ...
    const architecture = contract.roles?.architecture;
    const architectureProhibited = architecture?.capabilities?.prohibited ?? [];
    if (architecture?.writeScope !== "assigned-agent-work-files-and-project-documentation" || !(architecture?.capabilities?.optional ?? []).includes("workspace-write") || !["production-code-write", "test-write", "executable-config-write", "dependency-write", "user-decision", "overall-coordination"].every((item) => architectureProhibited.includes(item))) diagnostics.push(diag(contractFile, "ARCHITECTURE_WRITE_BOUNDARY", "architecture must be code-read-only with optional assigned documentation writes"));
    ...
    const review = contract.roles?.review;
    if (review?.writeScope !== "read-only" || !["production-write", "test-write", "product-test", "user-decision", "overall-coordination"].every((item) => (review?.capabilities?.prohibited ?? []).includes(item))) diagnostics.push(diag(contractFile, "REVIEW_WRITE_BOUNDARY", "Review must remain read-only and prohibited from implementation, Product Test, user decisions, and overall coordination"));
  ...
  return { ok: diagnostics.length === 0, diagnostics, summary: { skills: 1, roles: 4, mainResources: MAIN_RESOURCE_COUNT } };
}
