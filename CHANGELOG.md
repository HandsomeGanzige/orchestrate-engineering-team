...
- Added selectively disclosed execution-continuity guidance for terminal result barriers, supervisor/detach recovery, acceptance-format rejection, idempotent notifications, conservative parent-managed waves, and precise delivery status.
- Extended host-neutral LaunchPlan facts with backward-compatible result-delivery, dependency-barrier, and supervisor-continuation declarations; omitted facts normalize to `unknown` and trigger conservative orchestration.
