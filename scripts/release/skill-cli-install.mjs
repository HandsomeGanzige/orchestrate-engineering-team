...
export async function assertInstalledSkillSuite(targetRoot) { const names = (await readdir(path.join(targetRoot, ".agents/skills"), { withFileTypes: true })).filter((entry) => entry.isDirectory()).map((entry) => entry.name); assert.deepEqual(names, ["orchestrate-engineering-team"]); const result = await checkAgentProfiles(targetRoot); assert.equal(result.ok, true); return { skills: 1, roles: 4 }; }
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) runSkillCliInstall().then((summary) => console.log(`Skill install fixture passed: ${summary.skills} public Skill and ${summary.roles} roles.`)).catch((error) => { console.error(error.message); process.exitCode = 1; });
